...
PMF of X and Y:
\begin{align}
P(X = i) &= P(Y = i) = \frac{1}{5}, \quad i = 2, 3, 4, 5, 6
\end{align}
CDF of X and Y:
\begin{align}
F_X(k) &= F_Y(k) = \sum_{i=2}^{k} P(X = i) = \sum_{i=2}^{k} P(Y = i), \quad k = 2, 3, 4, 5, 6
\end{align}
The random variable Z represents the difference between the days Shyam and Ekta visit the shop.
Probability Mass Function (PMF) of Z:
\begin{align}
P(Z = k) &= P(X - Y = k) = \sum_{i=2}^{6} P(X = i, Y = i-k), \quad k = -4, -3, -2, -1, 0, 1, 2, 3, 4
\end{align}
Cumulative Distribution Function (CDF) of Z:
\begin{align}
F_Z(k) &= \sum_{i=-\infty}^{k} P(Z = i), \quad k = -4, -3, -2, -1, 0, 1, 2, 3, 4
\end{align}
\subsection*{Distribution of Z using Z-transform}
since x and y are independent \\by knowing pmf of x and y\\we can find the pmf of Z by using z transform 

\begin{align}
\pr{Z = z} &= \sum_{x} \sum_{y} \pr{X = x, Y = y} \cdot z^{-(x-y)}
\end{align}

\begin{figure}[!ht]
    \centering
        \includegraphics[width=\columnwidth]{figs/pmf_plot.pdf}
        \label{ PMF Plot}
    \end{figure}
 \begin{figure}[!ht]
        \centering
        \includegraphics[width=\columnwidth]{figs/cdf_plot.pdf}
        \label{ CDF Plot}
    \end{figure}
 \begin{figure}[!ht]
        \centering
